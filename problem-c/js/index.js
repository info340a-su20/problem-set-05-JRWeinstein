...
function createTaskItemElement(task) {
  ...
  elem.textContent = task.description;
...
function renderTaskList() {
  ...
  for (var i = 0; i < state.taskList.length; i++) {
    let liElem = createTaskItemElement(state.taskList[i]);
    document.querySelector('ol').appendChild(liElem);
  }
  renderInput();
}
...
function addNewTask() {
  let fIndex = state.taskList.length-1;
  let newTask = state.taskList[fIndex];
  newTask.id++;
  newTask.description = state.inputtedText;
  newTask.complete = false;
  state.taskList.push(newTask);
  state.inputtedText = "";
  // let newTask = {id:state.taskList.length+1,description:state.inputtedText,complete:false};
  // state.taskList.push(newTask);
  // state.inputtedText = "";
  renderTaskList();
}
...
inpElem.addEventListener('input', () => {
  state.inputtedText = inpElem.value;
  renderInput();
});
...
function renderInput() {
  let inpElem = document.querySelector('input');
  inpElem.value = state.inputtedText;
